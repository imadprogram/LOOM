...
<div class="bg-red-50 border-l-4 border-red-500 p-4 mb-8 rounded-r-lg shadow-sm flex items-start gap-4">
    ...
    <div>
        <h3 class="font-bold text-red-800">Review Required</h3>
        <p class="text-red-700 text-sm mt-1">There are currently <strong>3 unresolved reports</strong> waiting for admin action.</p>
    </div>
</div>
                    ...
                    @forelse ($newestAnnonces as $annonce)
                        ...
                        <tr class="hover:bg-gray-50 border-b border-gray-50 last:border-none transition">
                            <td class="p-4 py-5 w-16">
                                @if($annonce->image)
                                    <img src="{{ asset('storage/' . $annonce->image->file_path) }}" alt="{{ $annonce->title }}" class="w-10 h-10 rounded-xl object-cover shadow-sm">
                                @else
                                    <div class="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center shadow-inner">
