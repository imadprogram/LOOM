    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-x-auto">
        <table class="w-full text-left border-collapse min-w-[700px]">
            ...
            <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/80 transition-colors">
                        <td class="p-5">
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-12 h-12 rounded-xl bg-gray-100 border border-gray-200 overflow-hidden shrink-0">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">{{ $annonce->category->name }} •
                                        {{ $annonce->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="p-5">
                            ...
                            @else
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-100 text-red-700 text-xs font-bold">
                        ...
                        <td class="p-5 text-right space-x-2">
                            <!-- Toggle Status -->
                            @if ($annonce->status === 'active')
                                <form action="{{ route('deactivate.annonce', $annonce->id) }}" method="POST"
                                    class="inline">
                                    @csrf @method('PATCH')
                                    <button
                            ...
                            @else
                                <form action="{{ route('activate.annonce', $annonce->id) }}" method="POST" class="inline">
                                    @csrf @method('PATCH')
                                    <button
                                        class="text-xs font-bold text-green-600 bg-green-50 hover:bg-green-100 px-4 py-2 rounded-xl transition-all">
                                        Activate
                                    </button>
                                </form>
                            @endif
                            ...
                            <!-- Delete Permanently -->
                            <form id="delete-form-{{ $annonce->id }}"
                                action="{{ route('delete.annonce', $annonce->id) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
                                <button
                                    class="text-gray-400 hover:text-gray-600 p-2 rounded-xl hover:bg-gray-100 transition-all">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-10 text-center text-gray-400 font-medium italic">No listings found to
                            manage.</td>
                    </tr>
                @endforelse
            </tbody>
