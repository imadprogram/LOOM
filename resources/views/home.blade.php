...
@section('content')
    <div class="max-w-6xl mx-auto p-4 md:p-8">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
            ...
            <div
                class="bg-[#52c6be]/10 text-[#52c6be] px-4 py-2 rounded-full font-bold text-sm whitespace-nowrap self-start sm:self-auto">
                    ...
                    <div
                        ...
                        <div class="p-6">
                            <div class="flex justify-between items-start mb-2">
                                <h3
                                    class="text-xl font-bold text-gray-900 leading-tight group-hover:text-[#52c6be] transition-colors line-clamp-1">
                                    {{ $annonce->title }}</h3>
                                <span
                                    class="text-lg font-black text-[#52c6be] whitespace-nowrap ml-4">{{ $annonce->price }}€</span>
                            </div>
                            <p class="text-gray-500 text-sm mb-6 line-clamp-2">{{ $annonce->description }}</p>
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <div
                                        class="w-8 h-8 rounded-full bg-gray-200 overflow-hidden text-center flex items-center justify-center text-xs font-bold text-gray-400 uppercase">
                                        {{ substr($annonce->user->first_name ?? 'U', 0, 1) }}
                                    </div>
                                    <span
                                        class="text-sm font-medium text-gray-600 truncate max-w-[100px]">{{ $annonce->user->first_name ?? 'Unknown' }}</span>
                                </div>
