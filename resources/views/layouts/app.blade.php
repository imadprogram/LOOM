...
<body class="h-screen justify-center items-center">
    ...
    @auth
        @php
            $hasUnreadMessages = \App\Models\Message::where('receiver_id', auth()->id())->where('is_read', false)->exists();
        @endphp
        <a href="/messages" class="fixed bottom-8 right-8 bg-[#52c6be] text-white p-4 rounded-full shadow-lg shadow-[#52c6be]/30 hover:-translate-y-1 hover:shadow-[0_10px_20px_-5px_rgba(82,198,190,0.4)] transition-all z-50 group">
            <div class="relative">
                ...
                <span id="unread-dot" class="absolute -top-1 -right-1 w-3.5 h-3.5 bg-red-500 border-2 border-white rounded-full {{ $hasUnreadMessages ? '' : 'hidden' }}"></span>
            </div>
        </a>
